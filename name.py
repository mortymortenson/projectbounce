...
class Name(object):
    def __init__(self, symbol: str):
        ...
        self.vwap2m = vwap.TrailingVWAP(datetime.timedelta(minutes=2), datetime.timedelta(minutes=2)/10)
        self.vwap5m = vwap.TrailingVWAP(datetime.timedelta(minutes=5), datetime.timedelta(minutes=5)/10)
    ...
    def onTrade(self, time: datetime.datetime, price: float, size: float) -> None:
        ...
        self.vwap2m.onValue(time, price, size)
        self.vwap5m.onValue(time, price, size)
    ...
    def __str__(self) -> str:
        ...
        return (f"{self.symbol}"
                ...
                f" 2m: [ vwap: {self.vwap2m.getVWAP(now)} vol: {self.vwap2m.getVolume(now)} ]"
                f" 5m: [ vwap: {self.vwap5m.getVWAP(now)} vol: {self.vwap5m.getVolume(now)} ]"
